...
class SessionManager extends EventEmitter {
	...
	async create(key) {
		if (key) {
			this.log.debug('[sessions] Creating session with key "%s"', key);
		} else {
			this.log.debug('[sessions] Creating session without key');
		}
	...
	async persistAllUserData() {
		this.log.debug('[sessions] Persisting all user data');
	...
	async destroy(persist) {
		this.log.debug('[sessions] Destroying all sessions');
